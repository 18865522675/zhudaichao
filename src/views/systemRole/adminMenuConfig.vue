            ...
            <div class="cardBody flex-r">
                ...
                <div class="roleConfig-right" >
                    ...
                    <div class="marT10 flex-r" style="justify-content: flex-end">
                        ...
                        <el-button type="default"  class="reloadBtn resetBtns" @click="$router.go(-1)">取消</el-button>
                    </div>
                </div>
            </div>
    ...
    methods: {
      ...
      getRoleList(){
        this.$api.system.getRoleList().then((res)=>{
          ...
          if(res.data.length){
            ...
            this.getAdmin_ajax(this.adminId)
          }
        })
      },
      //获取数据
      get_ajax(roleId) {
        let url="getRoleTreeInfo";
        let params={};
        if(roleId==this.routeRoleId){
          url="getAdminTreeInfo";
          params={adminId:this.adminId}
        }else{
          params={roleId}
        }
        this.$api.system[url](params)
          .then(res => {
            this.clean_up(res.data);
          });
      },
      getAdmin_ajax(adminId){
        this.$api.system.getAdminTreeInfo({
          adminId
        })
          .then(res => {
            this.clean_up(res.data);
      ...
      saveConfig(){
        ...
        this.$api.system.editAdminMenu({
          ...
          this.$router.push({
            params:merge(this.$route.params,{'roleId':this.roleId})
          })
